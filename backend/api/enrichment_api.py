...
class EnrichmentRequest(BaseModel):
    ...
    condition: Optional[str] = Field(None, description="Item condition: good, damaged, obsolete, new")
...
async def record_item_enrichment(
    ...
    try:
        ...
        if not enrichment_data:
            raise HTTPException(
                status_code=400,
                detail="No enrichment data provided"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to record enrichment: {str(e)}"
        )
...
async def check_data_completeness(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to check completeness: {str(e)}"
        )
...
async def get_incomplete_items(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get incomplete items: {str(e)}"
        )
...
async def bulk_import_enrichments_endpoint(
    ...
    if current_user.get("role") not in ["admin", "supervisor"]:
        raise HTTPException(
            status_code=403,
            detail="Only admin/supervisor can perform bulk import"
        )
