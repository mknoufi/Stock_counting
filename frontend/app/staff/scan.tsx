...
export default function ScanScreen() {
  ...
  // Consolidated search state (keeping as local state for now)
  const [searchState, setSearchState] = React.useState({
    allItems: [] as Item[],
    searchResults: [] as Item[],
    showSearchResults: false,
  ...
  const { workflowState, updateWorkflowState, addSerialInput, removeSerialInput, updateSerialInput } = useWorkflowState();
  ...
  }, [watchedCountedQty, watchedReturnableDamageQty, watchedNonReturnableDamageQty, watchedMrp]);
  ...
  const updateWorkflowState = React.useCallback((updates: Partial<WorkflowState>) => {
    setWorkflowState((prev: WorkflowState) => ({ ...prev, ...updates }));
  }, []);
  ...
  }, [createSerialInput, workflowState.serialInputs]);
  ...
  const fuzzyMatch = (search: string, target: string): number => {
    ...
    const handleSearch = React.useCallback((query: string) => {
      ...
      searchTimeoutRef.current = setTimeout(async () => {
        try {
          ...
          const results = await ItemVerificationAPI.searchItems(query);
          updateUiState({ searchResults: results });
        } catch (error) {
          if (__DEV__) {
    ...
    const selectItemFromSearch = async (searchResult: SearchResult) => {
      ...
      const item: Item = {
        ...
        item_name: searchResult.item_name,
        barcode: searchResult.barcode,
        stock_qty: searchResult.stock_qty,
        mrp: searchResult.mrp,
        category: searchResult.category,
        warehouse: searchResult.warehouse,
        floor: searchResult.floor,
        rack: searchResult.rack,
      };
                  ...
                  <View style={styles.modalContent}>
                    ...
                    {itemState.currentItem && workflowState.existingCountLine && (
                      <View style={styles.addQtyInfo}>
                        <Text style={styles.addQtyItemName}>{itemState.currentItem.item_name}</Text>
